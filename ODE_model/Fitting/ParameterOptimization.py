...
if __name__ == '__main__':
   ...
   # Check if plateIDName exists in the data
    ...
    # Check if weights exists in the data
    weights = 'Num_cells'
    try:
        data[weights]
    except KeyError:
        sys.exit(message % (weights, odeFile))
    ...
    for cell_linei, celllinet in enumerate(obsStateVarList):
            ...
            for dosei, stresslevel in enumerate(doseList):
                ...
                for replicatei, replicateID in enumerate(plateID_list[cell_linei]):
                    ...
                    for tpi,tp in enumerate(tps):
                        ...
                        print('tpi = order of tp: ', tpi)
                        print('datasample = whole row: ', datasample)
                        ...
                        print('dataTempReal: expressions, ', dataTempReal)
            # Get data timepoints
        ...
        print('dfListReal: list of all timepoint expressions:', dfListReal)
    ...
    # by HELENE
    # add the weights in the same dimensions as dfTemp and dfReal
    data.sort_values(['statevariable', 'concentration', 'plate_id', 'timepoints'], inplace= True)

    plate_list = []
    dose_list = []
    rep_list = plateID_list[0]
    start = 0
    end = 5
    nrtp = 5
    weight_array = np.empty((len(doseList), len(rep_list), end - start))
    for d_idx, d in enumerate(doseList):
        for p_idx, p in enumerate(rep_list):
            timepoint_list = []
            for i in range(start, end, 1):
                row = data.iloc[i, :]
                timepoint_list.append(row[weights])
            weight_array[d_idx, p_idx, :] = timepoint_list
            start = start + 5
            end = end + 5

    #print('WEIGHT_array: ', weight_array)    
    #print('dfListReal', dfListReal)
    ...
    for cell_linei, celllinet in enumerate(obsStateVarList):
        ...
        if len(plateID_list) > 0 and len(doseList) > 0:
            ...
            dfWeight = np.array([]).reshape(len(plateID_list[cell_linei]), 0)
            dfTempReal = np.array([]).reshape(len(plateID_list[cell_linei]), 0)
            dfWeightReal = np.array([]).reshape(len(plateID_list[cell_linei]), 0)
            for dosei in np.arange(0, len(doseList), 1):
                ...
                dfWeight = np.hstack((dfWeight, weight_array[dosei]))
                dfWeightReal = np.hstack((dfWeightReal, weight_array[dosei]))
                print('comparison dfreal: ', dfReal )
                print('comparison weight_array: ', weight_array )
        ...
        print('tftemp', dfTemp)
        masked_dfTemp = ma.masked_invalid(dfTemp)
        masked_dfTempReal = ma.masked_invalid(dfTempReal)
        print('masked dftemp: ', masked_dfTemp)
        
        #print('DFTEMPREAL: ', dfTemp)
        print('DFweight: ', dfWeight) 
        print('DFTEMPREAL shape: ', dfTemp.shape)

        dfTempM = ma.average(masked_dfTemp, weights=dfWeight, axis=0)
        print('dftemp average: ', dfTempM)
        #dfTempM = dfTempM.data
        print('data back: ', dfTempM)
        #dfTempM = np.average(dfTemp, weights=dfWeight, axis = 0)
        dfTempMReal = ma.average(masked_dfTempReal, weights=dfWeight, axis=0)
        #dfTempMReal = np.average(dfTempReal, weights=dfWeight, axis = 0)
            ...
            if (len(doseList) > 1) & (len(obsStateVarList) > 1):
                ...
                for i, dose in enumerate(doseList):
                    for j, stateVar in enumerate(obsStateVarList):
                        ...
                        # Plot the data
                        # edit Helene
                        print('realtimepoints shape: ', realTimePoints.shape)
                        print('dataMreal shape: ', dataMReal[j, begin:begin + Nt].shape)
