...
def differencePreData(pLog, argP):
    ...
    if len(doseList) == 0 and len(plateID_list) == 0:
        ...
        print(dataObs)
        print('dataS:', dataS)
        difference = np.divide((dataObs - dataM), dataS, out=np.zeros_like(dataObs), where=~np.isnan(dataS))
        print("difference between:")
        print(difference)
        difference[np.isnan(dataS)] = ((dataObs - dataM)/0.6)[np.isnan(dataS)]
...
def multiesti(seed, argPplus):
    ...
    if testRun:
        ...
        try: # HELENE EDIT
            est = least_squares(differencePreData, pLog, jac=jacobian, bounds=boundub, args=(argP,), xtol=1.0e-5,
                        ftol=1.0e-5, gtol = 1.0e-13, verbose=2)
        except:
            print('OPTIMIZATION WENT WRONG FOR THIS SET - first time') 
        # Update parameters
	...
	# HELENE EDIT
        try:
            ...
            print('OPTIMIZATION WENT WRONG FOR THIS SET - second time')        # Retrieve the cost of cost function at estimation round 2
        ...
        while flagContinue and count < 10:
            ...
            try: # HELENE EDIT
                ...
                print('OPTIMIZATION WENT WRONG FOR THIS SET n')
